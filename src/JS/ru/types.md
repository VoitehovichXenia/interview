...
Примитивы (значения хранятся напрямую)
...
Ссылочные (хранится ссылка (указатель) на место в паямяти, где располагаются значения)
- **object**
...
- **Явное преобразование (Explicit Coercion / Type Casting)** разработчик намеренно указывает, что хочет изменить тип переменной.
```js
const a = 5;
let b = '10';

console.log(a + Number(b)) // явное преобразование типа
```
- **Неявное преобразование (Implicit Coercion / Type Coercion)** ЯП автоматически преобразует типы данных в процессе выполнения операции, часто в ситуациях, когда типе операндов не совпадают.
```js
console.log(5 - '3') // 2 
// '3' неявно преобразовалось в число
```